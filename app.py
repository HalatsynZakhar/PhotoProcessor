        ...
        if enable_bg_crop:
            ...
            if enable_crop:
                ...
                extra_crop_percent = st.slider(
                    ...
                    format="%.1f%%",
                    help="Дополнительно обрезать изображение на указанный процент после основной обрезки. Полезно для удаления лишних областей вокруг объекта или создания более компактной композиции. При 0% применяется только базовая обрезка по границам объекта."
                )
    ...
    with st.expander("4. Добавление отступов", expanded=should_expand_padding):
        ...
        if selected_padding_mode_key != 'never':
            ...
            pad_p = st.slider("Процент полей", 0.0, 200.0, 
                              ...
                              help="Размер добавляемых прозрачных полей в процентах от большей стороны изображения. Поля добавляются равномерно со всех четырёх сторон. Например, при значении 5% и изображении 1000x800 px будет добавлено 50 px прозрачного пространства с каждой стороны (5% от 1000). Большие значения полезны при создании коллажей или добавлении текста.")
